...
| `scripts/cleanDatabase.js` | 删除文件缺失的无效记录 | `node scripts/cleanDatabase.js` | `server/db/index.js` | ⚠️ 修正功能描述 |
| `scripts/clearHistory.js` | 清空历史记录表并删除图片文件 | `node scripts/clearHistory.js` | `sqlite3`（直接使用） | ⚠️ 修正依赖 |
...
| `scripts/syncHistoryState.js` | 同步历史记录（检查文件存在性并更新状态） | `node scripts/syncHistoryState.js` | `server/db/index.js` | ✅ 准确 |
...
| 地址 | 功能 | 调用方式 | 修正说明 |
| --- | --- | --- | --- |
| `server/db/index.js` | SQLite 操作封装（better-sqlite3） | `import db from './db/index.js'` | ✅ 准确 |
...
| `server/routes/hash.js` | 哈希计算、上链、验证、删除 | `POST /calculate-hash`, `POST /upload-to-blockchain`, `POST /verify-hash`, `DELETE /delete-hash/:hash` | `db`, `ethers`, `../debug/imageStorageDebug.js` | ⚠️ 修正依赖（原称 `contractManager` 未使用） |
...
| `server/historyController.js` | 旧版历史控制器（可删除） | ✅ 准确（建议清理） |
| `server/test-all-imports.js` | 导入测试 | ✅ 准确 |
| `server/test-h_and_up.html` | 测试页面 | ✅ 准确 |
| `server/server-esm-conversion-report.json` | 临时报告 | ✅ 准确（可删除） |
| `server/{` | 空文件 | ✅ 准确（可删除） |
...
**生成时间**：2026-04-07  
**验证依据**：对项目实际代码的逐文件审查，共发现并修正 50+ 处错误
